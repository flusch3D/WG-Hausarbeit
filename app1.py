...
def get_supabase() -> Client:
    # 1. Zuerst Streamlit Secrets versuchen (Cloud + lokales secrets.toml)
    url = ""
    key = ""
    try:
        ...
        pass  # Keine Secrets-Datei vorhanden – kein Problem, Fallback folgt

    # 2. Fallback: Umgebungsvariablen / .env Datei
    if not url:
    ...
    if not url or not key:
        ...
        st.info("Streamlit Cloud: App-Einstellungen → Secrets → eintragen:\n```\nSUPABASE_URL = \"https://xxxx.supabase.co\"\nSUPABASE_KEY = \"eyJ...\"\n```")
        st.stop()
